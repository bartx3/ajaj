...
from utils.prepare_fashion_mnist import DEFAULT_NPZ_PATH, attack_x_key, load_partitions


def to_loader(x: np.ndarray, batch_size: int, shuffle: bool) -> DataLoader:
    t_x = torch.from_numpy(x[:, None, :, :].astype(np.float32))
    return DataLoader(TensorDataset(t_x), batch_size=batch_size, shuffle=shuffle)


def anomaly_scores_from_sse(
    sse: np.ndarray,
    clean_sse: np.ndarray,
    mode: str = "deviation",
) -> np.ndarray:
    """deviation = |sse - mediana(clean)| — wykrywa też ataki z niższym SSE."""
    if mode == "sse":
        return sse
    if mode == "deviation":
        return np.abs(sse - float(np.median(clean_sse)))
    raise ValueError(f"Nieznany score_mode: {mode}")
...
def per_image_reconstruction_sse(
    model: nn.Module,
    x: np.ndarray,
    device: torch.device,
    batch_size: int = 128,
    ...
    """
    Score(x) = ||x - f(g(x))||^2 — suma kwadratów różnic po wszystkich pikselach (N,).
  """
    model.eval()
    loader = DataLoader(
    ...
    with torch.no_grad():
        for (bx,) in loader:
            bx = bx.to(device)
            recon = model(bx)
            sse = torch.sum((recon - bx) ** 2, dim=(1, 2, 3)).cpu().numpy()
            scores.append(sse)
    return np.concatenate(scores)


def eval_reconstruction_roc(
    model: nn.Module,
    clean: np.ndarray,
    attack: np.ndarray,
    device: torch.device,
    ...
    attack_sse = per_image_reconstruction_sse(model, attack, device, batch_size)
    ...
    stats = {
        ...
        "clean_mean_mse": float(np.mean(clean_sse)),
        "attack_mean_mse": float(np.mean(attack_sse)),
        "clean_median_mse": float(np.median(clean_sse)),
        "attack_median_mse": float(np.median(attack_sse)),
        "clean_std_mse": float(np.std(clean_sse)),
        "attack_std_mse": float(np.std(attack_sse)),
    }
    print(f"-- reconstruction SSE stats (score_mode={score_mode}) --")
...
def main() -> None:
    ...
    p.add_argument(
        "--score-mode",
        choices=("deviation", "sse"),
        default="deviation",
        help="deviation = |SSE - mediana(clean)|; sse = surowa suma kwadratów.",
    )
    args = p.parse_args()
    ...
    metrics = eval_reconstruction_roc(model, clean_x, attack_b, device, args.batch_size, args.score_mode)
    print("--- Wyniki autoenkodera ---")
    print(f"ROC-AUC ({args.score_mode}, clean vs attack_{args.attack_b}) = {metrics['roc_auc']:.4f}")

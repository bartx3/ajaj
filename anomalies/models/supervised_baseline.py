...
def run_baseline(
    ...
    *,
...
) -> dict[str, float | int | str | SmallCNN]:
    """Trenuje baseline (clean vs attack_a) i zwraca metryki ID/OOD."""
    if attack_a == attack_b:
        raise ValueError("attack_a i attack_b muszą być różne (OOD).")

    clean_x = data["clean_x"]
    key_a = attack_x_key(attack_a)
    key_b = attack_x_key(attack_b)
    if key_a not in data or key_b not in data:
        raise KeyError(f"{key_a} lub {key_b}")

    x_tr, y_tr, clean_te, attack_a_te, attack_b_te = build_train_test(
        clean_x, data[key_a], data[key_b], train_ratio, seed
    )
    ...
    if verbose:
        print(
            f"Baseline | train attack_{attack_a} ({attack_label(attack_a)}) "
            f"| test OOD attack_{attack_b} ({attack_label(attack_b)})"
        )
    ...
    id_metrics = eval_balanced_binary(model, clean_te, attack_a_te, device, batch_size)
    ood_metrics = eval_balanced_binary(model, clean_te, attack_b_te, device, batch_size)
...
def main() -> None:
    ...
    if args.attack_a == args.attack_b:
        raise SystemExit("--attack-a i --attack-b muszą być różne (OOD).")
    ...
    if args.save is not None:
        ...
        torch.save(metrics["model"].state_dict(), args.save)
        print(f"Saved supervised baseline state_dict to {args.save}")

    name_a = metrics["attack_a_label"]
    name_b = metrics["attack_b_label"]
    id_metrics = {"roc_auc": metrics["id_roc_auc"], "accuracy": 0.0, "balanced_accuracy": metrics["id_balanced_accuracy"]}
    ood_metrics = {"roc_auc": metrics["ood_roc_auc"], "accuracy": 0.0, "balanced_accuracy": metrics["ood_balanced_accuracy"]}
